...
install_github("pmartinezarbizu/pairwiseAdonis")
...
p_alpha <- plot_richness(Phyloseq_rarefy, measures = c("Observed","Shannon","Simpson"), color = "day")+
...
p_alpha_boxplot <- ggplot(data_alpha_diversity, aes(x= day, y= value, color = day),)+
...
p_Pcoa_BC
ggsave("PCoA_BC.png", p_Pcoa_BC, width=10, height = 8)
...
variable_tested <- cbind(c("Added Alkalinity","Time","Added Alkalinity*Time", "Treatment"))
Adonis_pvalues <- cbind(c(adonis_table_dalk_day$`Pr(>F)`[1:3],adonist_table_Treatment$`Pr(>F)`[1]))  %>%
  round(3)
...
write.table(pairwise_table_day,"pairwise_posthoc_Treatment.tsv", sep="\t", row.names = F, quote = T)
...
fviz_nbclust(as.data.frame(PCoA_ordination), method = "gap_stat", FUNcluster = kmeans)

Silhouette_plot
ggsave("Silhouette_plot.png",Silhouette_plot, height = 8, width = 10)
wss_plot
ggsave("Wss_plot.png", wss_plot, height = 8, width = 10)
...
meta_data_cluster_name <- meta_data_cluster %>%
  ...
  rename(Sample=name, x_coordinate=x, y_coordinate=y,cluster="case_when(...)")
...
p_kmeans_3
  ...
  ggsave("Kmeans_Plot_3.png",p_kmeans_3,width = 10, height = 8)  
...
draw the plots with just 2 clusters
```{r}
table_stacked_barplot <- OTU_Table_For_Plot(Phyloseq_rarefy,"phylum",3)
p_kmeans_2_phylum_barplot<- Make_Bar_Plot_return_Plot(table_stacked_barplot,meta_data_stackplot)+
  ggtitle("Microbial community Phylum level ~ clusters") +
  scale_y_continuous(expand = c(0,0))+
  labs(fill= "Phylum ")

ggsave("kmean_community_2_phylum.png", p_kmeans_2_phylum_barplot, height = 8, width = 12)

table_stacked_barplot <- OTU_Table_For_Plot(Phyloseq_rarefy,"order",6)
p_kmeans_2_order_barplot<- Make_Bar_Plot_return_Plot(table_stacked_barplot,meta_data_stackplot)+
  ggtitle("Microbial community order level ~ clusters") +
  scale_y_continuous(expand = c(0,0))+
  labs(fill= "Order")

ggsave("kmean_community_2_order.png", p_kmeans_2_order_barplot, height = 8, width = 12)
...
#centroid adonis ----
adonis_table_unifrac <- adonis2(unifrac_distance_weighted~d_Alk*day, data=meta_data_statistical_test, permutations = 1000)
...
variable_tested <- cbind(c("Added Alkalinity","Time","Added Alkalinity*Time", "Treatment"))
Adonis_pvalues <- cbind(c(adonis_table_unifrac$`Pr(>F)`[1:3],adonis_table_unifrac_treatment$`Pr(>F)`[1]))  %>%
  round(3)
...
pairwise_table_day_uni_table <- pairwise.adonis(unifrac_distance_weighted, meta_data_statistical_test$d_Alk) %>%
  select(pairs,p.value,p.adjusted) %>%
  rename("Pairs" = "pairs",
         "p_value" = "p.value",
         "p_value_adjusted"= "p.adjusted") %>%
...
```{r}
cca_results_dAlk <- cca(t(count_table_filtered)~d_Alk, meta_data)
cca_results_treatment <- cca(t(count_table_filtered)~Treatment, meta_data)
cca_results_day <- cca(t(count_table_filtered)~day, meta_data)


summary(cca_results_dAlk)
plot(cca_results_dAlk)
anova(cca_results_dAlk)
anova(cca_results_day)
anova(cca_results_treatment)

mod <- varpart(t(count_table_filtered), ~ d_Alk, ~ day, ~ Treatment, data = meta_data)
summary(mod)
plot(mod)

count_table_filtered_dist <- vegdist(t(count_table_filtered), "bray")

cca_results_day <- cca(t(count_table_filtered)~day, meta_data)
summary(cca_results_day)
plot(cca_results_day, display ="species")

rda_results_dAlk <- rda(t(count_table_filtered)~d_Alk*day, meta_data)

summary(rda_results_dAlk)
anova(rda_results_dAlk)
plot(rda_results_dAlk, display ="sites")
```
